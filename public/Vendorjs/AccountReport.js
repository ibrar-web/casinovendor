                ...
                function(response) {
                    if (response.data) {
                        ...
                        for (let i = 0; i < $scope.items.length; i++) {
                            $scope.deposit += JSON.parse($scope.items[i]['deposit']);
                            $scope.Bounceback += JSON.parse($scope.items[i]['Bounceback']);
                            $scope.Redeems += JSON.parse($scope.items[i]['Redeems']);
                            $scope.profit += JSON.parse($scope.items[i]['profit']);
                            $scope.payout += JSON.parse($scope.items[i]['payout']);
                        }
