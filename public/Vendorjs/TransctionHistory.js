app.controller("transctionhistory", function($scope, $filter, $http, $interval) {
    $scope.uname = uname;
    ...
    var initload = function() {
        ...
        var PipelineData = {};
        $http
            .post("./vendor/transctionhistory/init", JSON.stringify(PipelineData))
            .then(
                function(response) {
                    if (response.data) {
    ...
    $scope.pagedItems = [];
    $scope.currentPage = 0;
    $scope.header = ['Id', 'Account', 'Amount', 'Description', 'From', 'To', "Change From", 'Change To', 'Date'];
    $scope.sort = {
        sortingOrder: "id",
        reverse: false,
    };
